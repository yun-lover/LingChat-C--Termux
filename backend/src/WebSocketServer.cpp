...
int WebSocketServer::websocket_connect_handler(const mg_connection* conn, void* ws_server_ptr) { return static_cast<WebSocketServer*>(ws_server_ptr)->handle_websocket_connect(conn); }
void WebSocketServer::websocket_ready_handler(mg_connection* conn, void* ws_server_ptr) { static_cast<WebSocketServer*>(ws_server_ptr)->handle_websocket_ready(conn); }
int WebSocketServer::websocket_data_handler(mg_connection* conn, int flags, char* data, size_t data_len, void* ws_server_ptr) { return static_cast<WebSocketServer*>(ws_server_ptr)->handle_websocket_data(conn, flags, data, data_len); }
void WebSocketServer::websocket_close_handler(const mg_connection* conn, void* ws_server_ptr) { static_cast<WebSocketServer*>(ws_server_ptr)->handle_websocket_close(conn); }
int WebSocketServer::civetweb_error_log_handler(const mg_connection *conn, const char *message) { std::string msg = message; if (!msg.empty() && msg.back() == '\n') msg.pop_back(); std::cerr << "[CivetWeb Internal Error] " << msg << std::endl; return 0; }

// 构造函数会从配置中读取最大历史轮数，并用它来初始化 session_manager_
...
WebSocketServer::~WebSocketServer() { stop(); log_info("WebSocketServer 已销毁。"); }

void WebSocketServer::start() {
    if (ctx_) { log_error("WebSocketServer 已经在运行中。"); return; }
    ...
    if (getcwd(current_path_buffer, sizeof(current_path_buffer)) == nullptr) { throw std::runtime_error("无法获取当前工作目录。"); }
    std::string absolute_document_root = std::string(current_path_buffer) + "/" + doc_root;
    const char* options[] = { "listening_ports", port_str.c_str(), "document_root", absolute_document_root.c_str(), "num_threads", "5", nullptr };
    ...
    mg_set_websocket_handler(ctx_, "/websocket", websocket_connect_handler, websocket_ready_handler, websocket_data_handler, websocket_close_handler, this);
    log_info("WebSocket 端点已注册: /websocket");
}

void WebSocketServer::stop() { if (ctx_) { log_info("正在停止 WebSocket 服务器..."); mg_stop(ctx_); ctx_ = nullptr; std::lock_guard<std::mutex> lock(connection_mutex_); active_connection_ptr_ = nullptr; log_info("WebSocket 服务器已停止。"); } }
int WebSocketServer::handle_websocket_connect(const mg_connection* conn) { std::lock_guard<std::mutex> lock(connection_mutex_); if (active_connection_ptr_ != nullptr) { log_info("拒绝新连接: 已有活跃连接存在。"); return 1; } log_info("新 WebSocket 连接请求已授权，等待就绪..."); return 0; }
void WebSocketServer::handle_websocket_close(const mg_connection* conn) { const struct mg_request_info *ri = mg_get_request_info(conn); log_info("WebSocket 连接正在关闭 (ID: " + std::to_string((uintptr_t)conn) + "): " + std::string(ri->request_uri)); std::lock_guard<std::mutex> lock(connection_mutex_); if (active_connection_ptr_ == conn) { active_connection_ptr_ = nullptr; log_info("活跃连接已清空。服务器现在可以接受新连接。"); } }
...
int WebSocketServer::handle_websocket_data(mg_connection* conn, int flags, char* data, size_t data_len) {
    ...
    try {
        ...
        if (prompt_for_ai.empty()) return 1;
        ...
        for (std::sregex_iterator i = sentences_begin; i != sentences_end; ++i) {
            ...
            std::string audio_url = engine_.synthesizeSpeech(text_jp, config_.get("API", "VOICE_API_URL", ""));
        ...
        if (segments.empty()) {
            ...
            segments.push_back({{"expression", "default"}, {"action", ""}, {"text_cn", ai_raw_response}, {"audio_url", ""}});
        }
...
void WebSocketServer::send_websocket_message(mg_connection* conn, const std::string& message) { if (conn) { mg_websocket_write(conn, MG_WEBSOCKET_OPCODE_TEXT, message.c_str(), message.length()); log_info("发送 WebSocket 数据: " + message.substr(0, 200) + "..."); } }
void WebSocketServer::log_info(const std::string& message) const { std::cout << "[信息] WebSocketServer: " << message << std::endl; }
void WebSocketServer::log_error(const std::string& message) const { std::cerr << "[错误] WebSocketServer: " << message << std::endl; }
void WebSocketServer::log_warning(const std::string& message) const { std::cerr << "[警告] WebSocketServer: " << message << std::endl; }